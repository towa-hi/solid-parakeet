...
namespace Linework.FastOutline
{
    ...
    public class FastOutline : ScriptableRendererFeature
    {
        private class FastOutlinePass : ScriptableRenderPass
        {
            ...
            public bool Setup(ref FastOutlineSettings fastOutlineSettings, ref Material maskMaterial, ref Material outlineMaterial, ref Material clearMaterial)
            {
                ...
                foreach (var outline in settings.Outlines)
                {
                    ...
                    {
                        outline.AssignMaterial(outlineBase);
                    }
                }
                ...
                foreach (var outline in settings.Outlines)
                {
                    ...
                    outline.material.SetInt(CommonShaderPropertyId.BlendModeSource, srcBlend);
                    outline.material.SetInt(CommonShaderPropertyId.BlendModeDestination, dstBlend);
                    switch (outline.maskingStrategy)
                    {
                        case MaskingStrategy.Stencil:
                            outline.material.SetFloat(CommonShaderPropertyId.CullMode, (float) CullMode.Off);
                            break;
                        case MaskingStrategy.CullFrontFaces:
                            outline.material.SetFloat(CommonShaderPropertyId.CullMode, (float) CullMode.Front);
                            break;
                        default:
                            throw new ArgumentOutOfRangeException();
                    }
                    outline.material.SetColor(ShaderPropertyId.OutlineColor, outline.color);
                    outline.material.SetColor(ShaderPropertyId.OutlineOccludedColor,  outline.occlusion == Occlusion.WhenOccluded ? outline.color : outline.occludedColor);
                    outline.material.SetFloat(ShaderPropertyId.OutlineWidth, outline.width);
                    if (outline.extrusionMethod == ExtrusionMethod.ClipSpaceNormalVector)
                    {
                        outline.material.SetFloat(ShaderPropertyId.OutlineWidth, outline.width);
                        outline.material.SetFloat(ShaderPropertyId.MinOutlineWidth, outline.minWidth);
                    }
                    else
                    {
                        outline.material.SetFloat(ShaderPropertyId.OutlineWidth, outline.width * 0.015f);
                        outline.material.SetFloat(ShaderPropertyId.MinOutlineWidth, outline.minWidth * 0.015f);
                    }
                    if (outline.enableOcclusion) outline.material.EnableKeyword(ShaderFeature.Occlusion);
                    else outline.material.DisableKeyword(ShaderFeature.Occlusion);
                    if (outline.scaling == Scaling.ScaleWithDistance) outline.material.EnableKeyword(ShaderFeature.ScaleWithDistance);
                    else outline.material.DisableKeyword(ShaderFeature.ScaleWithDistance);
                    switch (outline.occlusion)
                    {
                        ...
                        case Occlusion.WhenOccluded:
                            outline.material.SetFloat(CommonShaderPropertyId.ZTest, (float) CompareFunction.GreaterEqual);
                            break;
        ...
        public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
        {
            ...
            var render = fastOutlinePass.Setup(ref settings, ref maskMaterial, ref outlineMaterial, ref clearMaterial);
